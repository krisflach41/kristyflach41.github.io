...
function emLoadDashboard() {
  ...
  fetch(API_BASE + '/email-center', {
    ...
    // Campaign performance panel — per-campaign analytics
    ...
    if (active.length === 0) {
      ...
      active.forEach(function(c) {
        ...
        h += '<div style="border-bottom:1px solid var(--border);padding:10px 12px;" id="emPerfRow_' + c.id + '">';
        // Header row — clickable to expand/collapse
        h += '<div style="display:flex;align-items:center;gap:10px;cursor:pointer;" onclick="emTogglePerfDetail(' + c.id + ')">';
        h += '<div style="width:8px;height:8px;border-radius:50%;background:#22c55e;flex-shrink:0;"></div>';
        h += '<div style="flex:1;font-size:13px;font-weight:600;color:var(--text-secondary);">' + c.name + '</div>';
        h += '<div style="font-size:10px;color:var(--text-muted);">' + stepCount + ' email' + (stepCount !== 1 ? 's' : '') + '</div>';
        ...
        h += '</div>';
        // Expandable detail — hidden by default, loaded on first expand
        h += '<div id="emPerfDetail_' + c.id + '" style="display:none;margin-top:10px;"></div>';
        h += '</div>';
      });
      el.innerHTML = h;
    }
...
function emTogglePerfDetail(campaignId) {
  ...
  if (detail.style.display === 'none') {
    // Collapse any other open ones
    document.querySelectorAll('[id^="emPerfDetail_"]').forEach(function(el) {
      if (el.id !== 'emPerfDetail_' + campaignId) {
        el.style.display = 'none';
        var otherId = el.id.replace('emPerfDetail_', '');
        var otherChevron = document.getElementById('emPerfChevron_' + otherId);
    ...
    if (!emPerfLoaded[campaignId]) {
      ...
      fetch(API_BASE + '/email-center', {
        ...
        // Link to full detail
        h += '<div style="text-align:right;margin-top:6px;"><button class="topbar-btn" style="font-size:10px;padding:3px 10px;" onclick="emSwitchTab(\'campaigns\');setTimeout(function(){emOpenCampaignDetail(' + campaignId + ')},200);">View Full Detail</button></div>';
